      ...
      <div className="h-full flex flex-col">
        ...
        <div className="p-6 border-b bg-orange-50">
          <div className="flex items-center justify-between">
            <h3 className="text-lg font-serif font-semibold text-stone-900">Your Cart</h3>
        ...
        <div className="flex-1 overflow-y-auto p-6">
          {items.length === 0 ? (
            ...
            <div className="space-y-4">
              {items.map((item) => (
                <div key={item.id} className="flex items-center space-x-4 p-4 bg-stone-50 rounded-xl">
                  {item.imageUrl ? (
                    <img
                      src={item.imageUrl}
                      alt={item.name}
                  ...
                  <div className={`w-16 h-16 bg-gradient-to-br from-orange-100 to-orange-200 rounded-lg flex items-center justify-center ${item.imageUrl ? 'hidden' : ''}`}>
                    <span className="text-2xl">🍴</span>
                  </div>
                  <div className="flex-1">
                    <h4 className="font-medium text-stone-900">{item.name}</h4>
                    <p className="text-gourmet-red font-semibold">${item.price.toFixed(2)}</p>
                  </div>
